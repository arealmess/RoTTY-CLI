...
class Program
{
  ...
  static readonly decimal NewYear = 2 * (decimal)Math.PI;
  ...
  static Dictionary<int, List<decimal>> Transits(decimal moon, int startYear = 1, int endYear = 4)
  {
    ...
    Dictionary<int, List<decimal>> events = new();
    ...
    while (thisYear <= endYear)
    {
      ...
      if (!events.TryGetValue(thisYear, out List<decimal>? value))
      {
        value = new List<decimal>(); events[thisYear] = value;
      } 
      value.Add(days * 180m / (decimal)Math.PI);
      lunarEclipses += 1;
    }
    return events;
  }
  ...
  {
    if (angles.Count < 2) return 0.0m;  
    angles = angles.Select(angle => angle == 0m ? 360m : angle).ToList(); 
    decimal averageTransits = angles.Skip(1).Select((angle, i) => angle - angles[i]).Average();
  ...
  static void Main()
  {
    ...
    if (isInputSeed)
    {
      ...
      try
      {
        if (input.StartsWith("-"))
        {
          ...
          else Console.WriteLine("¦ Error: Invalid decimal limit.");
          else if (command == "-r" && parts.Length == 2)
          {
            ...
            else Console.WriteLine("¦ Error: Invalid year range");
          }
          else if (command == "-t")
          {
            translateToText = !translateToText;
            if (translateToText) Colorify.WriteLine("Translation enabled", Colors.txtWarning);
            else Colorify.WriteLine("Translation disabled", Colors.txtWarning);
          }
          else Console.WriteLine("¦ Error: Invalid command");
        }
        else if (decimal.TryParse(input, out decimal moon))
        {
      ...
      catch (FormatException) { Console.WriteLine("¦ Error: Invalid input"); }
    }
    else
    {
      ...
      else
      {
        try
        {
          ...
          else
          {
            ...
            catch (FormatException) { Console.WriteLine("¦ Error: Invalid target angles."); }
          }
        }
    ...
    {
      List<string> lines = new(); List<string> line = new();
      int currentLength = 0;
      ...
      if (line.Any()) lines.Add(string.Join(", ", line));  
      return lines;
    }
    static string? ReadInput() { return Task.Run(() => Console.ReadLine()).Result; }
  }
}
